resource "openstack_networking_network_v2" "kube_cluster_network" {
...
resource "openstack_networking_subnet_v2" "kube_cluster_subnet" {
  ...
  network_id = openstack_networking_network_v2.kube_cluster_network.id
  cidr = var.cidr
  ip_version = 4
  dns_nameservers = ["1.1.1.1","8.8.8.8"]
}

resource "openstack_networking_router_v2" "kube_cluster_router" {
...
resource "openstack_networking_router_interface_v2" "nrif" {
  router_id = openstack_networking_router_v2.kube_cluster_router.id
  subnet_id = openstack_networking_subnet_v2.kube_cluster_subnet.id
}
